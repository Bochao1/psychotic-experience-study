...
fid<-vars_names[var]
df <- read.csv(paste("./sums/PE_", fid,".csv",sep = ''))
set.seed(888)
dim(df)
...
Final 
vars_names[var]
res[var,4:7]<-Final[var,]; res[var,8:11]<-Final[2,]; res[var,12:15]<-Final[3,]
WriteXLS::WriteXLS(res,'./res/Stable_8B_reverse_CCed.xls')
