...
vars_names_B
#for (var in 2:length(vars_names_B)){
  var=31
  grep(2040,vars_names_B )
  
geno<-read.table(paste('./raw/',fid_B[var],'.raw',sep=''),header = T)
geno$eid<-geno$FID
head(geno)
size<- ncol(geno)-7
geno<-geno[,-(1:6)]
...
G1.2 <- as.matrix(geno)
head(G1.2)
grep(fid[var],names(df))
names(df)[grep(fid_B[var],names(df))]
X.2<- as.matrix(df[,grep(fid_B[var],names(df))])
X.2<-ifelse(X.2=='Yes',1,0)
Y <- as.matrix(df$PE) 
...
write.csv(data, paste('sums/',fid_B[var],"_PE.csv",sep=''))
}


var=2
geno<-read.table(paste('./raw/',fid_Q[var],'.raw',sep=''),header = T)
geno$eid<-geno$FID
head(geno)
size<- ncol(geno)-7
geno<-geno[,-(1:6)]
...
size
  ...
  Y<- as.matrix(df[,grep(fid_Q[2],names(df))])
  X.2 <- as.matrix(df$PE) 
  G1.2<-as.matrix(geno) 
  # covariates: sex, age, assessment centre, chip, 40 genetic principal components
  sex<-as.matrix(df[,39])
  ...
  SNP<-as.matrix(row.names(data))
  colnames(SNP) <- c("SNP_ukb")
  write.csv(data, paste('sums/',fid_Q[var],"_PE.csv",sep=''))
}
